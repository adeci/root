	...
	ColumnLayout {
		...
		ColumnLayout {
			...
			RowLayout {
				...
				MullvadIcon {
					...
					crossed: root.vpnState === "disconnected" || root.vpnState === "error" || !root.installed
					color: {
				...
				ColumnLayout {
					...
					NText {
						Layout.fillWidth: true
						Layout.preferredHeight: headerDetailsHeightProbe.implicitHeight
						pointSize: Style.fontSizeS
					...
					NText {
						readonly property string ipv4: root.main?.currentLocation?.ipv4 || ""
						text: ipv4 || " "
						opacity: ipv4 ? 1 : 0
		...
		ColumnLayout {
			...
			NListView {
				...
				delegate: Rectangle {
					width: relayListView.availableWidth
					...
					RowLayout {
						...
						NText {
							...
							text: model.label
							pointSize: Style.fontSizeS
							color: rowMouse.containsMouse ? Color.mOnHover :
								(model.isCurrent ? Color.mOnPrimary : Color.mOnSurface)
							elide: Text.ElideRight
						}
						NText {
							...
							color: rowMouse.containsMouse ? Color.mOnHover :
								(model.isCurrent ? Color.mOnPrimary : Color.mOnSurfaceVariant)
						}
					}
		...
		ColumnLayout {
			...
			Rectangle {
				Layout.fillWidth: true
				Layout.preferredHeight: advancedContent.implicitHeight + Style.margin2L
			...
			NButton {
				Layout.fillWidth: true
				text: pluginApi?.tr("advanced.title")
				icon: root.advancedExpanded ? "chevron-down" : "chevron-right"
