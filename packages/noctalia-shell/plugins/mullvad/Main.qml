...
Item {
	...
	Process {
		...
		onExited: function (code) {
			if (code === 0) {
				root.lockdownMode = requestedState
				ToastService.showNotice(
					pluginApi?.tr("toast.title"),
					pluginApi?.tr(requestedState ? "toast.lockdown-on" : "toast.lockdown-off"),
					"shield"
				)
			} else {
				ToastService.showError(pluginApi?.tr("toast.title"), pluginApi?.tr("toast.lockdown-failed"), "alert-circle")
			}
			refreshTogglesState()
		}
	}
	...
	function setLockdown(on) {
		if (!root.installed || setLockdownProc.running) return
		setLockdownProc.requestedState = on
