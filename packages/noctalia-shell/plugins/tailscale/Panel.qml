...
Item {
  ...
  property real contentPreferredHeight: panelReady
    ? ((mainInstance?.tailscaleRunning ?? false) ? 700 : 160) * Style.uiScaleRatio
    : 0
  ...
  Rectangle {
    ...
    ColumnLayout {
        ...
        clip: true
          ...
          RowLayout {
            ...
            NIconButton {
              ...
              onClicked: mainInstance?.updateTailscaleStatus()
            }
          }
          ...
          Rectangle {
            ...
            visible: (root.mainInstance?.tailscaleRunning ?? false) && !root.isTerminalConfigured
          ...
          NListView {
            ...
            Layout.fillWidth: true
            Layout.fillHeight: true
            visible: root.mainInstance?.tailscaleRunning ?? false
            ...
            delegate: ItemDelegate {
                  id: peerDelegate
                  width: peerListView.availableWidth
                  implicitWidth: peerListView.availableWidth
                  ...
                  readonly property bool externalTailnet: peerData.ExternalTailnet || false
                  readonly property string peerOwner: peerData.OwnerName || ""
                  readonly property var primaryRoutes: peerData.PrimaryRoutes || []
                  readonly property bool hasNetworkIndicator: primaryRoutes.length > 0
                    || peerData.ExitNode || peerData.ExitNodeOption

                  function networkTooltip() {
                    var details = []
                    if (peerData.ExitNode)
                      details.push(pluginApi?.tr("panel.exit-node-selected"))
                    else if (peerData.ExitNodeOption)
                      details.push(pluginApi?.tr("panel.exit-node-available"))
                    if (primaryRoutes.length > 0)
                      details.push(pluginApi?.tr("panel.subnet-routes", { routes: primaryRoutes.join(", ") }))
                    return details.join("\n")
                  }
                  ...
                  contentItem: RowLayout {
                    ...
                    Rectangle {
                      ...
                      radius: width / 2
                      color: peerDelegate.peerOnline ? Color.mTertiary : Color.mOutline
                    }
                    ...
                    Item {
                      Layout.preferredWidth: Style.fontSizeM * 2 + Style.marginXS
                      ...
                      RowLayout {
                        ...
                        Item {
                          id: externalTailnetIndicator
                          Layout.preferredWidth: Style.fontSizeM
                          Layout.preferredHeight: Style.fontSizeM
                          visible: peerDelegate.externalTailnet
                          ...
                          HoverHandler {
                            onHoveredChanged: {
                              if (hovered) {
                                var text = peerDelegate.peerOwner
                                  ? pluginApi?.tr("panel.external-tailnet-owner", { owner: peerDelegate.peerOwner })
                                  : pluginApi?.tr("panel.external-tailnet")
                                TooltipService.show(externalTailnetIndicator, text, "left")
                              } else {
                                TooltipService.hide()
                              }
                            }
                        ...
                        Item {
                          ...
                          Layout.preferredWidth: Style.fontSizeM
                          Layout.preferredHeight: Style.fontSizeM
                          visible: peerDelegate.hasNetworkIndicator

                          NIcon {
                            anchors.centerIn: parent
                            icon: peerDelegate.primaryRoutes.length > 0 ? "route" : "globe"
                            pointSize: Style.fontSizeS
                            color: peerDelegate.peerData.ExitNode ? Color.mPrimary : Color.mOnSurfaceVariant
                            opacity: peerDelegate.peerData.ExitNode ? 1 : 0.65
                          }

                          HoverHandler {
                            onHoveredChanged: {
                              if (hovered)
                                TooltipService.show(networkIndicator, peerDelegate.networkTooltip(), "left")
                              else
                                TooltipService.hide()
                            }
                          }
                    ...
                    NText {
                      ...
                      color: Color.mOnSurfaceVariant
                      font.family: Settings.data.ui.fontFixed
                      Layout.preferredWidth: peerIpWidthProbe.implicitWidth
                      Layout.minimumWidth: peerIpWidthProbe.implicitWidth
      ...
      NButton {
        Layout.fillWidth: true
        visible: mainInstance?.tailscaleInstalled ?? false
