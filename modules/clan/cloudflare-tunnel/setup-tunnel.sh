...
# Wrapper for Cloudflare API calls with consistent options and auth
cf_api() {
  curl --max-time 30 --connect-timeout 10 -sf \
...
get_base_domain() {
  ...
  # Note: This simple approach works for standard TLDs (e.g., example.com)
  # but may not work correctly for multi-level TLDs (e.g., example.co.uk)
  echo "$hostname" | grep -oE '[^.]+\.[^.]+$'
}

# Step 1: Verify token
echo "Verifying API token..."
VERIFY_RESPONSE=$(cf_api "https://api.cloudflare.com/client/v4/user/tokens/verify")
...
if [ -z "$TUNNEL_ID" ]; then
  ...
  EXISTING_TUNNELS=$(cf_api "https://api.cloudflare.com/client/v4/accounts/$ACCOUNT_ID/cfd_tunnel?name=$TUNNEL_NAME")
...
for HOSTNAME in $HOSTNAMES; do
  ...
  ALL_RECORDS=$(cf_api "https://api.cloudflare.com/client/v4/zones/$ZONE_ID/dns_records?name=$HOSTNAME")
  ...
  if [ "$CNAME_COUNT" -gt 0 ]; then
    ...
    if [ "$CURRENT_TARGET" = "$TUNNEL_TARGET" ]; then
      ...
      cf_api -X PUT \
        ...
        --data "{\"type\":\"CNAME\",\"name\":\"$SUBDOMAIN\",\"content\":\"$TUNNEL_TARGET\",\"proxied\":true}" \
        > /dev/null
